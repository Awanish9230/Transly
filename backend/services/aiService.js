...
const ensureAudioFile = (filePath) => {
  return new Promise((resolve, reject) => {
    ...
    if (AUDIO_EXTS.includes(ext)) return resolve(filePath);
...
// ✅ Transcribe with Whisper (UTF-8 safe)
const transcribeAudio = (audioFilePath) => {
  return new Promise((resolve, reject) => {
    ...
    python.stdout.on("data", (data) => (output += data.toString("utf8")));
    python.stderr.on("data", (data) => (errorOutput += data.toString("utf8")));
